...
class Seat extends Component {
  ...
  setAlphabhetSeat = () => {
    ...
    const leftSideSeat = this.state.leftSideSeat.map((item) => `${seatAlphabhet}${item}`); // ["A1", "A2", ..., "A7"]
    const rightSideSeat = this.state.rightSideSeat.map((item) => `${seatAlphabhet}${item}`);
    this.setState({
